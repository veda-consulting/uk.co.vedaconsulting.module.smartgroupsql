...
class CRM_Contact_BAO_Query {
  ...
  /**
   * the groupId for LLR SmartGroup Query Customisation
   *
   * @var int
   */
  public $_groupId; //LLR Customisation 16-07-13 SRH
  ...
  function initialize() {
    ...
    // LLR Customisation 16-07-13 SRH
    // Start //
    foreach ($this->_params as $key => $whereConditions) {
        if ($whereConditions[0] == 'group') {
            $this->_groupId = key($whereConditions[2]);            
        }
    }
    // End //
  }
  ...
  function query($count = FALSE, $sortByChar = FALSE, $groupContacts = FALSE) {
    ...
    if (!empty($this->_whereClause)) {
      ...
      // LLR Customisation 16-07-13 SRH
      CRM_Utils_Hook::smartGroupWhereClause($this->_groupId, $this->_tables, $this->_whereTables, $contactID, $this->_whereClause);    
